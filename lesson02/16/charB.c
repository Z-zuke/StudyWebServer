...
int main()
{
    ...
    // 2. 以只读的方式打开 fifo1
    int fd1_r = open("fifo1", O_RDONLY);
    if (fd1_r == -1)
    {
        perror("open");
        exit(0);
    }
    printf("open fifo1 success, wait read out...\n");

    // 3. 以只写的方式打开 fifo2
    int fd2_w = open("fifo2", O_WRONLY);
    if (fd2_w == -1)
    {
        perror("open");
        exit(0);
    }
    printf("open fifo2 success, wait write in...\n");

    // 4. fork 子进程，父进程负责写，子进程负责读
    pid_t pid = fork();
    action(pid, fd2_w, fd1_r);
    // 5. 关闭文件描述符
    close(fd2_w);
    close(fd1_r);
